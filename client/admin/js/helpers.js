...
Template.adm_db.helpers({
	...
	createdAt: function () {
		var date = Prospero.findOne(this.__originalId).info.createdAt;
		var dateString = date.getDate() + '.' + (date.getMonth() + 1) + '.' + date.getFullYear();
		return dateString;
	},
	editedAt: function () {
		var date = Prospero.findOne(this.__originalId).info.last_edit;
		var dateString = date.getDate() + '.' + (date.getMonth() + 1) + '.' + date.getFullYear();
		return dateString;
	},
...
Template.admin_db_stats.helpers({
	createdAt: function () {
		var date = Prospero.findOne(this._id).info.createdAt;
		var dateString = date.getDate() + '.' + (date.getMonth() + 1) + '.' + date.getFullYear();
		return dateString;
	},
	editedAt: function () {
		var date = Prospero.findOne(this._id).info.last_edit;
		var dateString = date.getDate() + '.' + (date.getMonth() + 1) + '.' + date.getFullYear();
		return dateString;
	},
})
