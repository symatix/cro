...
SearchIndex = new EasySearch.Index({
  ...
  engine: new MongoDBEngine({
    ...
    selectorPerField: function (field, searchString, options) {
      ...
      const date = options.search.props.date;
      const dateSecond = options.search.props.dateSecond;
      ...
      const bendingMedium = options.search.props.bendingMedium;
      ...
      */
      ...
      if (sortBy) {
        ...
        if(type){
          basicSelector['basic.type'] = {"$regex" : ".*" + type + ".*", "$options" : "i"};
          //basicFields['type'] = 1;
          }
        if(date){
        ...
        if(bendingMedium){
          basicSelector['materials.original.bendingMedium'] = {"$regex" : ".*" + bendingMedium + ".*", "$options" : "i"};
          }
        if(materials){
        ...
        /*
        if(elementsDetermined){
          for (i = 0; i < methodsArr.length; i++){
            basicSelector['details.methods.'+methodsArr[i]] = {elements:{"$regex" : ".*" + elementsDetermined + ".*", "$options" : "i"}};
          }
        }
        if(formulaeDetermined){
          for (i = 0; i < methods.length; i++){
            basicSelector['details.methods.'+methodsArr[i]+'.formulae'] = {"$regex" : ".*" + formulaeDetermined + ".*", "$options" : "i"};
          }
        }
        if(pigmentsIdentified){
          for (i = 0; i < methods.length; i++){
            basicSelector['details.methods.'+methodsArr[i]+'.pigmentsDyes'] = {"$regex" : ".*" + pigmentsIdentified + ".*", "$options" : "i"};
          }
        }
        if(interpretation){
          for (i = 0; i < methods.length; i++){
            basicSelector['details.methods.'+methodsArr[i]+'.notes.interpretation'] = {"$regex" : ".*" + interpretation + ".*", "$options" : "i"};
          }
        }
        */
